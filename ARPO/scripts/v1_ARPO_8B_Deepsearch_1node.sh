...
PROJECT_NAME="qwen3_8B_dr"          # Modify experiment group (Qwen3-8B Deep Research)
EXPERIMENT_NAME="arpo_dr_8B"        # Modify experiment name
...
CONFIG_NAME="ppo_trainer_dr.yaml"   # Deep Research trainer config (different from reasoning's ppo_trainer.yaml)
...
MAX_PROMPT_LENGTH=2000              # Maximum prompt length (longer for deep search context)
...
TRAIN_FILES="${ABSOLUTE_PATH}/rl_datasets/hard_search_1k.parquet" # Modify training data path
VALID_FILES=["${ABSOLUTE_PATH}/rl_datasets/gaia_test.parquet","${ABSOLUTE_PATH}/rl_datasets/hle_test.parquet"] # Modify validation data path

# ============================ Model Configuration ============================
# Actor: HF checkpoint published by ARPO authors for 8B deep-search agent
ACTOR_MODEL_PATH="dongguanting/Qwen3-8B-ARPO-DeepSearch"
...
SEARCH_CACHE_PATH="${ABSOLUTE_PATH}/search_cache/search_cache_arpo_dr_8B.json" # Modify (per-experiment cache to avoid cross-run contamination)
