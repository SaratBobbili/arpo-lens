...
PROJECT_NAME="qwen3_8B" # Modify experiment group (Qwen3-8B Deep Research, mirrors ARPO 8B-DR project)
EXPERIMENT_NAME="echo_dr_8B" # validator profile c1 (plan/reason/answer HL; tool choice + payload LL), phase_order=[low_level, high_level]
...
MAX_PROMPT_LENGTH=2000              # Maximum prompt length (longer for deep search context, mirrors ARPO 8B-DR)
MAX_RESPONSE_LENGTH=10000           # Maximum response length (longer for multi-step search trajectories)

# Data file paths (1K Agentic Deep Search RL train set; GAIA + HLE for validation)
TRAIN_FILES="${ARPO_ROOT}/rl_datasets/hard_search_1k.parquet" # Modify training data path
VALID_FILES=["${ARPO_ROOT}/rl_datasets/gaia_test.parquet","${ARPO_ROOT}/rl_datasets/hle_test.parquet"] # Modify validation data path

# ============================ Model Configuration ============================
# Actor: HF checkpoint published by ARPO authors for 8B deep-search agent (no local SFT for this variant).
CHECKPOINT_DIR="/scratch/project/prj-02-llm-reasoning-shakkottai/saratb/ECHO/sft"
ACTOR_MODEL_PATH="${CHECKPOINT_DIR}/checkpoints/Qwen3-8B"
...
SEARCH_CACHE_PATH="${ARPO_ROOT}/search_cache/search_cache_echo_dr_8B.json" # Per-variant cache for v1 with phase_order=[low_level, high_level] on the 8B deep-search dataset
...
# Training parameters
TOTAL_EPOCHS=5                      # Total training epochs (matches ARPO 8B-DR; deep-search needs more epochs over the 1K train set)
SAVE_FREQ=5                        # Save frequency
TEST_FREQ=5                        # Test frequency

# ============================ Path Configuration ============================
# Save path
CHECKPOINT_DIR="/scratch/project/prj-02-llm-reasoning-shakkottai/saratb/ECHO/DeepSearch"
...
BRIGHTDATA_API_KEY="f75663d4-caf9-432a-baf1-dec27a13625a" # Bright Data API token; set manually in terminal before launch
...
BRIGHTDATA_TIMEOUT=90                         # Per-HTTP-call read timeout (s) to api.brightdata.com. Brightdata SERP tail latency is ~30-60s+ under concurrent rollout load, so 90 absorbs the tail and avoids spurious retries.
# ============================ Preparation ============================
...
python3 -m recipe.echo.main_echo \
    ...
    actor_rollout_ref.rollout.tools.call_limit=6 \
