...
class OpenAISearchTool(BingSearchTool):
    """
    OpenAI web search tool. Reuses BingSearchTool's caching infrastructure
    (file locking, async writes, cache merging) but calls OpenAI's Chat
    Completions API with a search-enabled model instead of Brightdata/Bing.
    """
    ...
    def __init__(
        ...
        **kwargs,
        ...
        Args:
            ...
            model: Search-enabled model (gpt-5-search-api, gpt-4o-search-preview, etc.)
            max_results: Max number of result paragraphs returned
            result_length: Max characters per result paragraph
            cache_file: Shared cache file path (same file can be shared with BingSearchTool)
            async_cache_write: Whether to write cache asynchronously
            cache_refresh_interval: Seconds between cache-file freshness checks
            max_retries: OpenAI client retry count on 429/5xx (exponential backoff)
            max_concurrent_requests: Per-process cap on in-flight API calls (semaphore)
        """
        super().__init__(
            api_key=api_key,
        ...
        self._openai_client = OpenAIClient(api_key=api_key, max_retries=max_retries)
        self._model = model
    ...
    def execute(self, query: str, timeout: int = 60) -> str:
        """
        Execute search: cache lookup → OpenAI web search → cache update.
        Output format matches BingSearchTool (Page N: snippet).
        """
        query = query.replace('"', '')
        self._check_cache_update()
        ...
        # Semaphore limits concurrent API calls per process to avoid 429 bursts
        ...
        result = self._format_openai_results(text)
    ...
    def _format_openai_results(self, text: str) -> str:
        """Split OpenAI synthesized response into Page N: snippet format."""
        if not text.strip():
            return "No search results found."
        ...
        formatted = []
        for idx, para in enumerate(paragraphs[:self._max_results], 1):
            formatted.append(f"Page {idx}: {para[:self._result_length]}")
        return "\n".join(formatted)
...
if __name__ == "__main__":
    ...
    # --- OpenAISearchTool test ---
    openai_key = os.environ.get("OPENAI_API_KEY", "<your_openai_key>")
    openai_tool = OpenAISearchTool(
        api_key=openai_key,
        cache_file=cache_file,
    )
    print(f"\n[OpenAI] Searching for: {query}")
    result3 = openai_tool.execute(query)
