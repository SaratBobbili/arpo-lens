"""ALTERNATING-GRPO actor: role-masked GRPO over two phases, each with its own
optimizer.

No knowledge of the response term. `DataParallelECHOActor` in echo_dp_actor.py
subclasses this and fills the hooks at the bottom to add ECHO Algorithm 1.
"""
import logging
import os
...
class DataParallelPhaseActor(DataParallelPPOActor):
    """Two-phase GRPO. `phase_optims`/`phase_batch_sizes` are keyed by phase name."""
    ...
    def update_policy(self, data: DataProto):
        ...
        # HYPERGRADIENT seam: subclass parses its own meta_info here.
        self._update_begin(data, phase)
        ...
        for _k in self._extra_select_keys(data):
        ...
        self._on_batch_selected(selected, data, phase)

        # HYPERGRADIENT leaves p.grad = -g_resp here; the loop below accumulates
        # -g_dir on top of it. ALTERNATING-GRPO returns None and clears as usual.
        response_grad, _response_metrics = self._compute_response_gradient(
            dataloader, phase,
            temperature=temperature, mini_batch_size=mini_batch_size,
            micro_batch_size_per_gpu=micro_batch_size_per_gpu,
            use_dataproto_batches=use_dataproto_batches,
            use_aepo_clip=use_aepo_clip, use_sign_cond_clip=use_sign_cond_clip,
        )
        ...
        for epoch in range(self.config.ppo_epochs):
            for batch_idx, data in enumerate(dataloader):
                ...
                if self._should_zero_grad(response_grad):
        ...
        self._after_update(phase, metrics)
        return metrics


    # --- hooks for HYPERGRADIENT -------------------------------------------------
    # ALTERNATING-GRPO implements each as a no-op. DataParallelECHOActor overrides
    # them; nothing in this class may reference phases.response.

    def _update_begin(self, data, phase):
        """Called once per update, before anything is selected."""

    def _extra_select_keys(self, data):
        """Batch keys the subclass needs to reach the micro-batches."""
        return []

    def _on_batch_selected(self, selected, data, phase):
        """Called after select()/chunk(), before any gradient work."""

    def _compute_response_gradient(self, dataloader, phase, **ctx):
        """Return (grad_buffer_or_None, metrics) to be accumulated before g_dir."""
        return None, {}

    def _should_zero_grad(self, response_grad):
        """False when a subclass has pre-loaded p.grad and g_dir must accumulate."""
        return True

    def _on_response_grad_consumed(self, response_grad, metrics):
        """Called after the last backward; return the new value for response_grad."""
        return response_grad

    def _before_optimizer_step(self, phase, metrics):
        """Called between the last backward and the optimizer step."""

    def _after_optimizer_step(self, phase, grad_norm):
        """Called with the grad norm the step actually used."""

    def _after_update(self, phase, metrics):
        """Called once per update, after the final zero_grad()."""
