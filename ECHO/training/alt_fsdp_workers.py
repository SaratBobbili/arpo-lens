"""ALTERNATING-GRPO worker: one FSDP actor with a per-phase optimizer and scheduler.

No knowledge of the response term. `EchoActorRolloutRefWorker` in echo_fsdp_workers.py
subclasses this and adds the Algorithm-1 weight-snapshot RPCs.
"""
import itertools
...
class PhaseActorRolloutRefWorker(ActorRolloutRefWorker):
    """Two-phase GRPO worker. The actor class comes from _build_actor()."""
    ...
    def _build_actor(self):
        """The ALTERNATING-GRPO actor. HYPERGRADIENT overrides this to pass the
        weight-snapshot callbacks Algorithm 1 lines 2/14 need."""
        return DataParallelPhaseActor(
