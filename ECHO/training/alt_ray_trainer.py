"""ALTERNATING-GRPO trainer: role-masked GRPO over two phases in a nested cycle.

No knowledge of the response term. `RayECHOTrainer` in echo_ray_trainer.py
...
class RayAlternatingGRPOTrainer(RayPPOTrainer):
    """Two-phase role-masked GRPO. N_LL low-level iterations then one high-level,
    repeated for N_HL cycles. Each phase owns its optimizer and batch sizes."""
    ...
    def _next_batch_dict(self, phase_name: str):
        ...
        HYPERGRADIENT overrides this: Algorithm 1 draws distinct query groups for the
        follower and the leader (C.3/C.4), so it pulls a new batch every iteration.
        """
    ...
    def fit(self):
        ...
        self._after_fit(num_ll_iters, logger, progress_bar)
    ...
    # ALTERNATING-GRPO implements each as a no-op. RayECHOTrainer overrides them;
    # nothing in this class may reference phases.response.
    ...
    def _open_round(self) -> None:
        """Called at the top of every cycle, before the first low-level iteration."""

    def _close_cycle(self) -> None:
        """Called after the high-level iteration closes a cycle."""

    def _after_fit(self, num_ll_iters: int, logger, progress_bar) -> None:
        """Called once after the training loop, before the final report."""
