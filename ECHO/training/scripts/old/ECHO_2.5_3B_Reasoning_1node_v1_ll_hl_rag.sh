...
SEARCH_CACHE_PATH="${ECHO_TOP}/search_cache/search_cache_echo_3B.json" # Per-variant cache for v1 with phase_order=[low_level, high_level]
...
RAG_SOFT_FALLBACK=True               # On miss, hit Brightdata via inner BingSearchTool and append (q,result) to sidecar.
RAG_REQUEST_TIMEOUT=30               # HTTP read timeout (s) for sidecar /retrieve and /add.
