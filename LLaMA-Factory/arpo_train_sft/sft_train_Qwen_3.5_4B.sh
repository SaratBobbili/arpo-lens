#!/bin/bash
# ECHO cold-start SFT: echo_dataset -> dataset/echo_sft_new.jsonl (see dataset_info/dataset_info.json).
# Per-example system text (tool-use + reasoning format) lives in the dataset `system` column.
# Recipe (lr, epochs, <result> loss masking, checkpointing) comes from yaml/qwen.yaml.
