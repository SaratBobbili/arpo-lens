...
# -------------------- Editable Run Config --------------------
# Usage: bash run_echo_math_4qa_hf_single_job.sh <experiment_name>
#   e.g. bash run_echo_math_4qa_hf_single_job.sh echo7B_sft5e8_alt_grpo
# Resolves to ${CHECKPOINT_ROOT}/<experiment_name>/best_checkpoint/hf
#
# Override prefix / served name / recipe via env:
#   CHECKPOINT_ROOT=... REASON_MODEL_NAME=... TRAINING_RECIPE_PATH=...

if [[ $# -lt 1 || -z "${1:-}" ]]; then
  echo "usage: $0 <experiment_name>" >&2
  echo "  loads \${CHECKPOINT_ROOT}/<experiment_name>/best_checkpoint/hf" >&2
  exit 1
fi
EXPERIMENT_NAME="$1"
...
CHECKPOINT_ROOT="${CHECKPOINT_ROOT:-/scratch/project/prj-02-llm-reasoning-shakkottai/saratb/ECHO/checkpoints}"
TRAINING_RUN_DIR="${CHECKPOINT_ROOT}/${EXPERIMENT_NAME}"
...
REASON_MODEL_NAME="${REASON_MODEL_NAME:-${EXPERIMENT_NAME}}"
...
if [[ ! -d "$TRAINING_RUN_DIR" ]]; then
  echo "ERROR: training run dir not found: $TRAINING_RUN_DIR" >&2
  exit 1
fi
...
echo "[0/5] Experiment: $EXPERIMENT_NAME"
